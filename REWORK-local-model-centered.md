...
- **Phase 0 (MLX spike) — BUILDS, isolated; not yet run.** `spike/` standalone package (NOT in the main build) benchmarks title→tags latency vs the <500 ms band. **Full MLX graph resolves AND compiles to an executable** (mlx-swift 0.29.1, pinned mlx-swift-examples 2.29.1 — `main` stopped exposing `MLXLLM`/`MLXLMCommon`; `generate()` disambiguated to the `[Int]` overload). **Remaining manual step:** `swift run -c release spike mlx-community/Llama-3.2-1B-Instruct-4bit` (downloads the model, needs release for real numbers). Then add constrained decode + prefix-cache reuse before wiring a real `MLXOnDeviceLLM: OnDeviceLLM` into the main package.

### Remaining (next sessions)
- **Phase 0 finish:** run the spike, get 1B/3B numbers, add constrained decode + prefix-cache reuse, then wire a real `MLXOnDeviceLLM: OnDeviceLLM` into the main package.
