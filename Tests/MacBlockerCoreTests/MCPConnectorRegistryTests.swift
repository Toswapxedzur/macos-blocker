...
final class MCPConnectorRegistryTests: XCTestCase {
    ...
    func testProtectedAppDataPathClassification() {
        // Non-sandboxed app: plain Application Support is NOT guarded — only real
        // sandbox containers are.
        XCTAssertFalse(MCPConnectorRegistry.isProtectedAppDataPath("Library/Application Support/Code"))
    ...
    func testEveryCatalogClientIsSilentlyRegisterable() {
        // No catalog client lives in a sandbox container, and the app is not
        // sandboxed, so every installed client auto-registers at launch with no
        // prompt (the AI Tool Connections UI was removed 2026-09-23).
        for connector in MCPConnectorRegistry.defaultCatalog {
            XCTAssertTrue(connector.isSilentlyRegisterable, "\(connector.id) should be silently registerable")
        }
    }

    func testIsInstalledSilentlySeesAppSupportPaths() throws {
        // App Support is no longer a guarded path, so silent detection sees a
        // client present only via its App-Support directory.
        ...
        XCTAssertTrue(registry.isInstalledSilently(cursor), "silent detection now sees the App-Support path")
    }

    func testLaunchSweepConnectsAppSupportClients() throws {
        MCPConnectorRegistry.isLaunchAutoConnectEnabled = true
        defer { MCPConnectorRegistry.isLaunchAutoConnectEnabled = false }
        // VS Code and Claude Desktop store their config under Application Support,
        // which a non-sandboxed app writes with no prompt — so the launch sweep
        // now connects them automatically.
        ...
        XCTAssertTrue(
            ...
            "launch sweep writes VS Code's config automatically"
        )
        XCTAssertTrue(registry.isConnected(try XCTUnwrap(registry.connector(id: "vscode"))))
