(() => {
  ...
  function localModelWorkspace() {
    ...
    const panel = (model) => {
      ...
      const recordCount = (dataset?.records || []).filter((record) => sourcePlatforms.has(record.platformID) && record.review === "approved" && (record.origin === "manual" || record.origin === "llmAssist") && record.treeRevision === model.treeRevision).length + (dataset?.creatorClassifications || []).filter((classification) => sourcePlatforms.has(classification.platformID) && classification.review === "approved" && (classification.origin === "manual" || classification.origin === "llmAssist") && classification.treeID === model.treeID && classification.treeRevision === model.treeRevision).reduce((count, classification) => count + (dataset?.collectedEntries || []).filter((entry) => entry.platformID === classification.platformID && entry.creatorID === classification.creatorID).length, 0);
