...
- **Folders:** `Sources/` (`VaultClassifierApp`, `VaultClassifierCore`, `VaultClassifierLLM`, `VaultClassifierEval` (+ `abtest`), `Cllama`, `VaultLocalHubNativeHost`, smoke targets `VaultGroundingSmoke` / `VaultFullLoopSmoke` / `VaultLLMEngineSmoke`), `Tests/`, `native-host/` (native-messaging manifests + entitlements), `docs/architecture/`, `spike/` (throwaway experiment package), `dist/`.
- **Planning docs:** `REWORK-local-model-centered.md`, `ROADMAP-per-type-granularity.md`, `PHASE3-BUILD-PLAN.md`; datasets `eval-set.json`, `mini-ocr.json`.
